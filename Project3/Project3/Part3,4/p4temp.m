...
figure(55) ;
%     MyGUIHandle.handle65 = plot(0,0,'b.');
...
% ========================================================================
    ...
    for i = 1:N_imu-1  % Index of Vehicel Pose
        ...
        Xdr(:,i+1) = PredictVehiclePose(Xdr(:,i), yaw_rate1(i), speed(i) ,Dt);
 %%  Landmark 
        if j == 1
            landmark = ProcessFirstScan( dataL.Scans(:,j), MyGUIHandle, j, Xdr(:,i) ); 
            v = 0;
            [OOIs] = ProcessScan( dataL.Scans(:,j), MyGUIHandle, j, Xdr(:,i), landmark ,v);
        end
 %% Laser 
 ...
 %% EKF
           ...
           Jn = [ [ Dt*cos(Xe(3)), 0 ] ; 
            ...
            Xe_History(:,i) = Xe ;
%             set(MyGUIHandle.handle65,'xdata',i,'ydata',Xe_History(4,:));
            % =======================================
