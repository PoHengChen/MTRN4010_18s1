...
%         Xdr(:,i+1) = PredictVehiclePose(Xdr(:,i), yaw_rate(i), speed(i) ,times_imu(i+1)-times_imu(i));
        if times_imu(i) > times_laser(j)
            ...
            [OOIs] = ProcessScan( dataL.Scans(:,j), MyGUIHandle, j, Xdr(:,i), landmark );
...
%     xx = landmark.Centers(1,landmark_HR);
...
function [OOIs] = ProcessScan(scan,mh,j,Pose,landmark)
mask1FFF = uint16(2^13-1);
maskE000 = bitshift(uint16(7),13)  ; % shift 0111 to left 13 bits
...
OOIs = ExtractOOIs(ranges,intensities, X, Y) ;
 if OOIs.N > 0
    OOIs = PlotOOIs(mh,OOIs,intensities,j,Pose); % Obtain Global coordinate
    OOIs = PrintID(mh,OOIs,landmark); % Data association to obtain right index of OOIs
 else
    OOIs = OOIs;
    OOIs.ID = [];
 end
...
function OOIs = PlotOOIs(mh,OOIs,intensities,i,Pose) % this funciton return global coordinate
