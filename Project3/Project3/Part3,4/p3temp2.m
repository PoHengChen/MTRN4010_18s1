...
%         Xdr(:,i+1) = PredictVehiclePose(Xdr(:,i), yaw_rate(i), speed(i) ,times_imu(i+1)-times_imu(i));
        if times_imu(i) > times_laser(j)
            ...
            GOOIs = ProcessScan( dataL.Scans(:,j), MyGUIHandle, j, Xdr(:,i), landmark );
            % ================= KF ==================
           ...
           Jn = [ [ Dt*cos(Xe(3)), 0 ] ; 
            ...
            % ????????????????????????????
            [MasuredRanges, MasuredBearings] = GetMeasurementsFomNearbyLandmarks(Xe,GOOIs)
%             disp(i);
            if GOOIs.N > 0
...
% =========================================================================
function [ranges,bearings] = GetMeasurementsFomNearbyLandmarks(X,GOOIs)
    N = GOOIs.N;
    d = 0.46;
    if GOOIs.N>0
        dx = GOOIs.Centers(1,:) - X(1) - d*cos(X(3));
        dy = GOOIs.Centers(2,:) - X(2) - d*sin(X(3));
...
function GOOIs = ProcessScan(scan,mh,j,Pose,landmark)
mask1FFF = uint16(2^13-1);
maskE000 = bitshift(uint16(7),13)  ; % shift 0111 to left 13 bits
...
OOIs = ExtractOOIs(ranges,intensities, X, Y) ;
 if OOIs.N > 0
    ...
    GOOIs = PrintID(mh,GOOIs,landmark);
 else
    GOOIs = OOIs;
    GOOIs.ID = [];
 end
