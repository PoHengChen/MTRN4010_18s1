...
%% ================================= EKF Initialise======================
    ...
    %% process model noise
    % discrepency between real system and nominal model
    std_PM_X = 0.01;
    std_PM_Y = 0.01;
    std_PM_heading = 1*pi/180;
    %% input noise
...
%%
    % assume the initial model is perfectly known
    P = zeros(3,3) ;
    ...
    Q_Process_Model = diag( [ (std_PM_X)^2 ,(std_PM_Y)^2 , (std_PM_heading)^2]) ; % Q_model 
...
%% ========================================================================
 ...
 %% EKF 
       ...
       Jn = [ Dt*cos(Xe(3)), 0  ;               % Jacobian matrix of input
              ...
              0            , Dt ] ;         
        % ===== Prediction =====
        P = J*P*J' + Ju*Pu*Ju' + Dt^2*Q_Process_Model;
        ...
        if OOIs.N > 0
            for u = 1:length(OOIs.ID)
                ...
                eDD = sqrt( eDX^2 + eDY^2 ) ; %   so : sqrt( (xu-x)^2+(yu-y)^2 ) 
