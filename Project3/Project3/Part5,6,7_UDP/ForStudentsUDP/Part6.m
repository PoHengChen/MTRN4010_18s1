...
function Part6()
 ...
 %% =================== EKF =====================
    J = [ 1,  0,  -cur.Dt*cur.speed*sin(cur.Xe(3)),0 ;  0,  1,   cur.Dt*cur.speed*cos(cur.Xe(3)) ,0;   0,  0,  1  ,cur.Dt*(-1);0,0,0,1] ; % Jacobian matrix of model
   Jn = [ cur.Dt*cos(cur.Xe(3)), 0  ;   cur.Dt*sin(cur.Xe(3)), 0  ; 0 , cur.Dt ;0,0] ;  % Jacobian matrix of input       
    % 
    Q = cur.Dt^2*cur.Q_Process_Model + Jn*cur.Q_Input*Jn';
    cur.P = J*cur.P*J'+ Q ; % new covariance, P(K+1|K) = J*P(K|K)*J'+Q ;

    % ===== Prediction =====
    cur.Xe = PredictVehiclePose(cur.Xe,cur.yaw_rate,cur.speed,cur.Dt) ;
    ...
    if OOIs.N > 0
        for u = 1:length(OOIs.ID)
            ...
            H = [  -eDX/eDD , -eDY/eDD  ,0 , 0 ; eDY/(eDD^2), -eDX/(eDD^2)  ,-1,0 ] ;
    ...
    cur.i = cur.i+1;
 %% =========== Plot ====================
    set(MyGUIHandle.handle65,'xdata',1:size(cur.Xe_History(4,:),2),'ydata',cur.Xe_History(4,:)*180/pi);
    disp(cur.Xe(4));
