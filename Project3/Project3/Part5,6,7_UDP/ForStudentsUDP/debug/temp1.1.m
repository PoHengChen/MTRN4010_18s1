...
function main()
 ...
 load('IMU_dataC.mat') ;load('Speed_dataC.mat') ;
 global yaw_rate;
 global speeds;
...
figure(2); clf;
hold on;
plot(1:size(speeds,2),yaw_rate(:),'r');
plot(1:size(speedx,2),yawratex(:),'b');
...
%         cur.yaw_rate = -double(uint8(x(14)))*0.02*(pi/180);
       ...
       cur.speed_a = [cur.speed_a,cur.speed];
       cur.yaw_rate_a = [ cur.yaw_rate_a , cur.yaw_rate];
        ...
        cur.times_imu = time;
        cur.time_a = [cur.time_a,cur.times_imu];
