...
if OOIs.N > 0
    GOOIs = PlotOOIs(mh,OOIs,intensities,j,Pose); % getting green plot
    % ========================= Print OOI id =============================
    abc = [];
    for k = 1:GOOIs.N % start with certain ooi(green plot)
        ...
        for h = 1:landmark.N % check the distance with each landmark respectively
            X = [landmark.Centers(:,h)'; GOOIs.Centers(:,k)'];
            temp = pdist(X);
            if temp < min
               min = temp;
               index = h;
    ...
    if ~isempty(abc)
       for n = 1:length(abc)
           if abc(n) == 1
               set(mh.handle7,'Position',[double(landmark.Centers(1,abc(n))),double(landmark.Centers(2,abc(n)))],'String',abc(n)); 
%            else 
%                set(mh.handle7,'Position',[0,0],'String',0);
           end
%            if abc(n) == 2, set(mh.handle8,'Position',[double(landmark.Centers(1,abc(n))),double(landmark.Centers(2,abc(n)))],'String',abc(n)); end
%            if abc(n) == 3, set(mh.handle9,'Position',[double(landmark.Centers(1,abc(n))),double(landmark.Centers(2,abc(n)))],'String',abc(n)); end
%            if abc(n) == 4, set(mh.handle10,'Position',[double(landmark.Centers(1,abc(n))),double(landmark.Centers(2,abc(n)))],'String',abc(n)); end
%            if abc(n) == 5, set(mh.handle11,'Position',[double(landmark.Centers(1,abc(n))),double(landmark.Centers(2,abc(n)))],'String',abc(n)); end
       end
    end
end
    % ====================================================================
