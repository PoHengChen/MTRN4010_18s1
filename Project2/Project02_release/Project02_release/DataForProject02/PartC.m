...
function MyProgram()
    ...
    Pose(:,1) = [0 0 pi/2]; 
    j = 1; % index of laser(update slower, low frequency)
    for i = 1:N_imu-1 % index of IMU(update faster, high frequency)
...
X = -cos(angles).*ranges;   % inversed Laser convention of X axes
Y = sin(angles).*ranges;  
...
function PlotOOIs(mh,OOIs,i,Pose)
