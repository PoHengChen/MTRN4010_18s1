...
gg = [];% store shoestest distance in each gen
...
for g=1:PSO.G,% PSO generation
    ...
    for n=1:PSO.N,% PSO particles
        ...
        for t=0:time.dt:time.T,
            [ds]=FindDistance(car,real);
            vel=evalfis(ds,fis_vel);
            [dq]=FindAngular(car,real);
            ang=evalfis(dq,fis_ang);
            [car]=CarNow(car,time,vel,ang);
        end;
        
        err=[car.x-real.x car.y-real.y AngleWrap(car.q-real.q)];% pose error
...
figure(5); clf(); hold on; zoom on;
xlabel('generation');ylabel('Shortest distance');
plot(1:PSO.G,gg);
